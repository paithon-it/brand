...
Ogni generatore espone `NOME`, `TITOLO` e `costruisci() -> Figura`. Dove
finiscono le figure lo decide il consumatore, che passa la cartella a
`scrivi()`: nel libro lo fa `animazioni/svg/genera.py`.
"""
...
INK = "#1A1A1A"
CREAM = "#F8F5EE"
FG_MUTED = "#5A524A"     # --pt-fg-muted di tokens.css
BORDER = "#E2DCC9"
...
# Dove finiscono le figure lo decide il consumatore: il libro le vuole in
# book/figures/, il sito altrove. Il provino invece è cache di lavoro e sta
# bene ovunque, quindi resta deciso qui.
...
def scrivi(nome: str, titolo: str, fig: Figura, dest: Path,
           provino: bool = True) -> Path:
    """Scrive la figura in `dest` dopo averla verificata.
    ...
    if "<script" in testo:
        raise ValueError(f"{nome}: niente script negli SVG")
